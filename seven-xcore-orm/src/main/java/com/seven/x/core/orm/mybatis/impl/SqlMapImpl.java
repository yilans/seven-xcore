...
public class SqlMapImpl implements SqlMap {
	...
	static Logger log = LoggerFactory.getLogger(SqlMapImpl.class);
	...
	public SqlMapImpl(DataSource dataSource, SqlSessionFactory sqlSessionFactory, PersistenceExceptionTranslator persistenceExceptionTranslator) {
	...
	public SqlMapSession startSession(boolean isBatch) {
		SqlSession sqlSession = isBatch ? getSqlSession(this.dataSource, ExecutorType.BATCH) : getSqlSession(this.dataSource, this.isReuse ? ExecutorType.REUSE : ExecutorType.SIMPLE);
		return new SqlMapSessionImpl(sqlSession, this.sqlSessionFactory, this.persistenceExceptionTranslator);
	}

	private SqlSession getSqlSession(DataSource dataSource, ExecutorType executorType) {
		...
		if (log.isDebugEnabled()) {
			log.debug("Opened SqlSession [" + sqlSession + "] for iBATIS operation");
		}
	...
	private <T> T callBack(Transaction<T> transaction) throws DataAccessException {
		...
		SqlSession sqlSession = getSqlSession(this.dataSource, this.isReuse ? ExecutorType.REUSE : ExecutorType.SIMPLE);
		
		try {
			T t = transaction.transact(sqlSession);
	...
	public <T> T queryForObject(final String name, final Object param) throws DataAccessException {
		return (T) callBack(new Transaction<Object>() {
			public Object transact(SqlSession paramAnonymousSqlSession) {
				return paramAnonymousSqlSession.selectOne(name, param);
			}
	...
	public void update(String name, Object param, int expected) throws DataAccessException {
		int i = update(name, param);
		if (i != expected)
			throw new JdbcUpdateAffectedIncorrectNumberOfRowsException(name, expected, i);
	}
